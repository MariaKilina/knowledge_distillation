...
def resnet8(path='', pretrained=False, progress=True, **kwargs):
    model = ResNet(BasicBlock, [3, 0, 0, 0], **kwargs)
    if pretrained:
        model = model.load_state_dict(torch.load(path))
        model.eval()
    return model
...
def resnet34(path='', pretrained=False, progress=True, **kwargs):
...
def resnet101(path='', pretrained=False, progress=True, **kwargs):
